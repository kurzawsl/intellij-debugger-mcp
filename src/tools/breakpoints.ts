...
export const setBreakpointTool = defineTool({
  ...
  handler: async ({ file, line, condition, projectId }, ctx) => {
    const resolvedProjectId = await ctx.resolveProjectId(projectId);
    const result = await ctx.client.addBreakpointToProject(
      resolvedProjectId,
      file,
      line,
      condition
...
export const removeBreakpointTool = defineTool({
  ...
  handler: async ({ file, line, projectId }, ctx) => {
    const resolvedProjectId = await ctx.resolveProjectId(projectId);
    const result = await ctx.client.removeBreakpointFromProject(
      resolvedProjectId,
      file,
      line
    );
    return textContent(
      result.success ? `Breakpoint removed from ${file}:${line}` : result.message
    );
  },
});
...
export const toggleBreakpointTool = defineTool({
  ...
  handler: async ({ file, line }, ctx) => {
    ...
    return textContent(
      result.success ? `Breakpoint toggled at ${file}:${line}` : result.message
    );
  },
});
